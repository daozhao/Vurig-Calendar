...
-(void)selectDate:(int)date {
    ...
    [comps setDay:date];
    NSDate *selectDateTemp = [gregorian dateFromComponents:comps];
    if ( beginDate && [beginDate compare:selectDateTemp] == NSOrderedDescending  ) {
        return;
    }
    if ( endDate && [endDate compare:selectDateTemp] == NSOrderedAscending ) {
        return;
    }
...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        [colors addObject:[UIColor colorWithHexString:kVRGCalendarTextColor]];
    }
...
{       
    ...
    CGRect rectArrowLeft = CGRectMake(0, 0, 50, kVRGCalendarViewTitleHeight);
    CGRect rectArrowRight = CGRectMake(self.frame.size.width-50, 0, 50, kVRGCalendarViewTitleHeight);
    
    //Touch either arrows or month in middle
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        if ( (beginDate && [beginDate compare:self.currentMonth.firstDayInMonth] == NSOrderedAscending) || (!beginDate) ){
            [self showPreviousMonth];
        }
    } else if (CGRectContainsPoint(rectArrowRight, touchPoint)) {
        if ( (endDate && [endDate compare:self.currentMonth.lastDayInMonth] == NSOrderedDescending) || (!endDate) ){
            [self showNextMonth];
        }
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetFillColorWithColor(context,[UIColor colorWithHexString:kVRGCalendarBackgroundColor].CGColor);
    CGContextFillPath(context);
    
    //Arrows
    int arrowSize = kKNCalendarArrowHeight;
    int xmargin = kKNCalendarArrowOffsetSide;
    int ymargin = kKNCalendarArrowOffsetTop;
    ...
    if ( (beginDate && [beginDate compare:self.currentMonth.firstDayInMonth] == NSOrderedAscending) || (!beginDate) ){
        ...
        CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarArrowColor].CGColor);
        CGContextFillPath(context);
    }
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarGridBackgroundColor].CGColor);
    CGContextFillPath(context);
    
    //Grid white lines
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarBackgroundColor].CGColor);
    CGContextBeginPath(context);
    ...
    //Grid dark lines
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarGridDarkLinesColor].CGColor);
    CGContextBeginPath(context);
    ...
    CGContextSetFillColorWithColor(context, 
                                   [UIColor colorWithHexString:kVRGCalendarTextColor].CGColor);
    ...
    NSLog(@"currentMonth:%@ previousMonth:%@",self.currentMonth,previousMonth);
    NSLog(@"weekbegin:%@ weekend:%@",[NSDate dateStartOfWeek],[NSDate dateEndOfWeek]);
    NSLog(@"month begin:%@ month end:%@",self.currentMonth.firstDayInMonth,self.currentMonth.lastDayInMonth);
    ...
    if ( beginDate && [beginDate month] == [previousMonth month] && [beginDate year] == [previousMonth year]) {
        beginBlock = firstWeekDay -(prevMonthNumDays - beginDate.day ) - 1 ; //(prevMonthNumDays-firstWeekDay)+1;
    }
    if ( beginDate && [beginDate month] == [currentMonth month] && [beginDate year] == [currentMonth year]) {
        beginBlock = [beginDate day] + firstWeekDay - 1;
    }
    if ( endDate && [endDate month] == [currentMonth month] && [endDate year] == [currentMonth year]) {
        endBlock = [endDate day] + firstWeekDay - 1;
    }
    if ( endDate && [endDate month] == [nextMonth month] && [endDate year] == [nextMonth year]) {
        endBlock = currentMonth.numDaysInMonth + firstWeekDay + endDate.day -1;
    }
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (i<firstWeekDay) { //previous month
            ...
            NSString *hex = (isSelectedDatePreviousMonth) ? kVRGCalendarTextColor : kVRGCalendarPrevOrNextTextColor;
            ...
            NSString *hex = (isSelectedDateNextMonth) ? kVRGCalendarTextColor : kVRGCalendarPrevOrNextTextColor;
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        } else { //current month
            // isCurrentMonth = YES;
            targetDate = (i-firstWeekDay)+1;
            NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth) ? kVRGCalendarPrevOrNextTextColor : kVRGCalendarTextColor;
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        }
        
        if ( i < beginBlock+1 ){
             NSString *hex = kVRGCalendarDisableTextColor;
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextAddRect(context, rectangleGrid);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarSelectedTextBackgroundColor].CGColor);
            CGContextFillPath(context);
            CGContextSetFillColorWithColor(context,[UIColor colorWithHexString:kVRGCalendarSelectedTextColor].CGColor);
        } else if (todayBlock==i) {
            CGRect rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
            CGContextAddRect(context, rectangleGrid);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarTodayBackgroundColor].CGColor);
            CGContextFillPath(context);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:kVRGCalendarTodayTextColor].CGColor);
        }
        
        [date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight)
                withFont:kKNCalendarNumberFont
           lineBreakMode:UILineBreakModeClip
               alignment:UITextAlignmentCenter];
    }
...
-(id)init {
    ...
    if (self) {
        ...
        isAnimating=NO;
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(34, 0, kVRGCalendarViewWidth, kVRGCalendarViewTitleHeight)];
        [self addSubview:labelCurrentMonth];
        labelCurrentMonth.backgroundColor= [UIColor colorWithHexString:kVRGCalendarBackgroundColor];
        labelCurrentMonth.font = kKNCalendarTitleFont;
        labelCurrentMonth.textColor = [UIColor colorWithHexString:kVRGCalendarTitleLabelColor];
